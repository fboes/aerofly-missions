...
export class Mission {
  ...
  fromGarminFpl(gpl: GarminFpl): Mission {
    ...
    if (gpl.departureRunway && !this.findCheckPointByType(MissionCheckpoint.TYPE_DEPARTURE_RUNWAY)) {
      ...
      cp.lon_lat = this.origin_lon_lat.getRelativeCoordinates(
        0.5,
        Number(gpl.departureRunway.replace(/\D+/g, "")) * 10 + this.origin_lon_lat.magnetic_declination
      );
    ...
    if (gpl.destinationRunway && !this.findCheckPointByType(MissionCheckpoint.TYPE_DESTINATION_RUNWAY)) {
      const cp = new MissionCheckpoint();
      cp.lon_lat = this.destination_lon_lat.getRelativeCoordinates(
        0.5,
        Number(gpl.destinationRunway.replace(/\D+/g, "")) * 10 + 180 + this.destination_lon_lat.magnetic_declination
      );
